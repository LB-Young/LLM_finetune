...
target_dir_list = ['/mnt/xxxxx/Young_alpaca/']
...
model_path = '/mnt/xxxxxxxxxx/chatglm3-6b/'
...
for k, v in device_map_dict.items():
    if k == 'transformer.embedding.word_embeddings':
        model.transformer.embedding.word_embeddings = model.transformer.embedding.word_embeddings.to(f'cuda:{v}')
    if k.find("transformer.encoder.layers") != -1:
        sub_value = int(k.replace("transformer.encoder.layers.", ""))
        model.transformer.encoder.layers[sub_value] = model.transformer.encoder.layers[sub_value].to(f'cuda:{v}')

    if k == "transformer.output_layer":
        model.transformer.output_layer = model.transformer.output_layer.to(f'cuda:{v}')
